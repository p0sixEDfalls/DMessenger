...
bool GetMyExternalIP2(const CAddress& addrConnect, const char* pszGet, const char* pszKeyword, unsigned int& ipRet)
{
    ...
    while (RecvLine(hSocket, strLine))
    {
        if (strLine.empty())
        {
            ...
            //strLine = "70.86.96.218:80";//wxString(strLine).Trim();
...
bool GetMyExternalIP(unsigned int& ipRet)
{
    ...
    char* pszGet = new char[138];
    char* pszKeyword = new char[4];
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        if (nHost == 1)
        {
			...
			memcpy(pszGet, "GET /ip.php HTTP/1.1\r\nHost: www.ipaddressworld.com\r\nUser-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\nConnection: close\r\n\r\n", 138);
			memcpy(pszKeyword, "IP:", 4);
        }
        else if (nHost == 2)
        {
			...
			pszGet = new char[128];
			pszKeyword = new char[9];

			memcpy(pszGet, "GET / HTTP/1.1\r\nHost: checkip.dyndns.org\r\nUser-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\nConnection: close\r\n\r\n", 128);
			memcpy(pszKeyword, "Address:", 9);
        }
...
bool StartNode(string& strError)
{
    ...
    if (GetMyExternalIP3(addrLocalHost))
    {
