...
const uint256 hashGenesisBlock("feef9c73ec35a757262aa8bee0d7116b2cad8cd226839b79233f1f447028066c");
...
extern ThreadSafeVector<event> vEvents; 
uint160 myID;
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return -1; //return max(0, (COINBASE_MATURITY+20) - GetDepthInMainChain());
}
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
	...
	if (pfrom)
	{
		foreach(const CTransaction& tx, pblock->vtx)
		{
			if (tx.IsToMe())
			{
				vEvents.lock();
				vEvents.pushBack(make_pair(NEW_CNCT, tx.GetIPAdress()));

				printf("New dialog for messenger from: %02x\n", tx.GetIPAdress());
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        assert(block.hashMerkleRoot == uint256("4806f917a26c76092107f42c35e4a269f2278b423cea0a2a2c7fbdd7c77f31a1"));
...
{
	uint256 maxTarger;
	maxTarger.SetHex("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
	return maxTarger - 1;
}